...
class Packet:
    ...
    def encode(self):
        raw = struct.pack("<QBBBBHccqiiIIxxxxxxxxiiII", self.id, self.event_type, self.transfer_type, self.endpoint_number, self.device_address, self.bus_id, bytes([self.setup_flag]), bytes([self.data_flag]), self.ts_sec, self.ts_usec, self.status, len(self.data), len(self.data), self.interval, self.start_frame, self.xfer_flags, self.ndesc)
        raw = bytearray(raw)
